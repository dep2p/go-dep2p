...
// FIX: 在启动 goroutine 之前调用 shutdownEnd.Add(1)，避免数据竞争
func (s *Server) mainloop() {
	if s.ipv4conn != nil {
		s.shutdownEnd.Add(1) // FIX: 移到 goroutine 启动之前
...
// FIX: 移除 shutdownEnd.Add(1)，改在 mainloop 中调用
func (s *Server) recv4(c *ipv4.PacketConn) {
	if c == nil {
		s.shutdownEnd.Done() // 提前退出也要 Done
		return
	}
	buf := make([]byte, 65536)
	// s.shutdownEnd.Add(1) // FIX: 已移到 mainloop
