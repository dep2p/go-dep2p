# pkg/crypto

> DeP2P 密码学工具包

## 概述

`pkg/crypto` 提供 DeP2P 的核心密码学功能，包括密钥生成、签名验证、序列化和安全存储。

## 支持的密钥类型

| 类型 | 枚举值 | 公钥大小 | 私钥大小 | 签名大小 | 说明 |
|------|--------|----------|----------|----------|------|
| Ed25519 | 2 | 32 字节 | 64 字节 | 64 字节 | 默认推荐 |
| Secp256k1 | 3 | 33 字节 | 32 字节 | 64 字节 | 区块链兼容 |
| ECDSA (P-256) | 4 | 33 字节 | 32 字节 | 64 字节 | 标准曲线 |
| RSA | 1 | 变长 | 变长 | 变长 | 传统兼容 |

## 快速开始

### 生成密钥对

```go
import "github.com/dep2p/go-dep2p/pkg/crypto"

// 生成 Ed25519 密钥对（推荐）
priv, pub, err := crypto.GenerateKeyPair(crypto.KeyTypeEd25519)
if err != nil {
    log.Fatal(err)
}

// 生成 Secp256k1 密钥对（区块链兼容）
priv, pub, err := crypto.GenerateKeyPair(crypto.KeyTypeSecp256k1)
```

### 签名和验证

```go
// 签名
data := []byte("message to sign")
sig, err := crypto.Sign(priv, data)

// 验证
valid, err := crypto.Verify(pub, data, sig)
if valid {
    fmt.Println("签名有效")
}
```

### 序列化

```go
// 序列化公钥
pubBytes, err := crypto.MarshalPublicKey(pub)

// 反序列化公钥
pub, err := crypto.UnmarshalPublicKeyBytes(pubBytes)

// 序列化私钥
privBytes, err := crypto.MarshalPrivateKey(priv)

// 反序列化私钥
priv, err := crypto.UnmarshalPrivateKeyBytes(privBytes)
```

### PeerID 派生

```go
import "github.com/dep2p/go-dep2p/pkg/types"

// 从公钥派生 PeerID
peerID, err := crypto.PeerIDFromPublicKey(pub)
fmt.Println(peerID) // 12D3KooW...
```

### 密钥存储

```go
// 创建文件系统密钥存储（加密）
ks, err := crypto.NewFSKeystore("/path/to/keys", []byte("password"))

// 存储密钥
err = ks.Put("my-node-key", priv)

// 获取密钥
priv, err := ks.Get("my-node-key")

// 列出所有密钥
ids, err := ks.List()
```

## 文件结构

```
pkg/crypto/
├── doc.go              # 包文档
├── key.go              # 核心接口和工厂函数
├── errors.go           # 错误定义
├── ed25519.go          # Ed25519 实现
├── secp256k1.go        # Secp256k1 实现（纯 Go）
├── ecdsa.go            # ECDSA 实现
├── rsa.go              # RSA 实现
├── signature.go        # 签名结构
├── marshal.go          # 序列化
├── peerid.go           # PeerID 派生
└── keystore.go         # 密钥存储
```

## 安全特性

- **常量时间比较**：防止时序攻击
- **加密存储**：使用 AES-GCM + Argon2id
- **安全清零**：`SecureZero()` 函数清除敏感数据
- **CSPRNG**：使用加密安全随机源

## 与约束规范的对齐

本包实现遵循以下规范：

- [密钥格式规范](../../design/02_constraints/protocol/L1_identity/key_format.md)
- [签名规范](../../design/02_constraints/protocol/L1_identity/signature.md)
- [pkg/proto/key/key.proto](../proto/key/key.proto)

## 测试覆盖率

```
coverage: 83.0% of statements
```

## 相关文档

- [pkg/types](../types/) - 公共类型定义
- [pkg/proto/key](../proto/key/) - Protobuf 密钥定义
