...
| **网络弹性** | 网络切换/设备睡眠后自动恢复 | 系统级网络监听 + 自动恢复 ✅ |
| **路径健康** | 多路径（IPv4/IPv6/Relay）自动选择 | PathHealth 管理器 ✅ |
| **地址发现** | 多源地址融合（PortMap/STUN/Local） | 可达性协调器 ✅ |

#### 网络弹性能力（已实现）

DeP2P 通过学习 iroh 的网络弹性设计，已实现以下能力：

| 能力 | 说明 | 状态 |
|------|------|------|
| **系统级网络监听** | 主动感知网络变化（Wi-Fi 切换、设备睡眠） | ✅ 已实现 |
| **单飞地址发现** | 避免并发竞态，确保地址发现稳定 | ✅ 已实现 |
| **增强 NetReport** | NAT 类型判定、Relay 延迟测量 | ✅ 已实现 |
| **多源地址融合** | PortMap/STUN/Local/Configured 统一管理 | ✅ 已实现 |
| **PortMapper Watcher** | 外部地址变化事件驱动 | ✅ 已实现 |
| **路径健康管理** | 多路径健康监控和自动切换 | ✅ 已实现 |

> **详细分析**: 参见 [iroh 设计分析](../../design/discussions/DISC-IROH-DESIGN-ANALYSIS.md)

#### 区块链增强能力（规划中）

DeP2P 正在增强以下能力以更好地支持区块链场景：

| 能力 | 说明 | 优先级 |
|------|------|--------|
| **消息速率跟踪** | 根据对端能力动态调整请求大小，防止过载 | P0 |
| **入站/出站比例控制** | 确保有足够的出站连接以保持主动发现能力 | P0 |
| **IP 限制机制** | 防止同一 IP 段的节点过多占用连接资源（Sybil 攻击防护） | P0 |
| **拨号调度器** | 智能管理拨号任务，区分静态/动态拨号 | P0 |
| **静态节点自动重连** | 确保与关键节点 (Bootstrap/Validator) 的连接稳定 | P0 |
| **节点数据库持久化** | 重启后保留已知节点信息，加速启动 | P1 |
| **Capabilities 协商** | 连接建立时协商支持的协议版本 | P1 |

> **详细分析**: 参见 [区块链增强需求分析](../../design/implementation/BLOCKCHAIN_ENHANCEMENT_REQUIREMENTS.md)
...
realm, _ := node.Realm("mychain-mainnet")
_ = realm.Join(ctx)
